	...
	final void add1400to1499()
	{
		...
		new ChoiceAdventure( 1406, "Drippy House on the Prairie", "The Dripping Trees" )
		{
			void setup()
			{
				...
				new Option( 2, "add 7 drippy bats to this zone" )
					.turnCost( 1 );
				new Option( 3, "2-4 driplets" )
					.turnCost( 1 )
					.attachItem( ItemPool.DRIPLET, 1, AUTO );
				new Option( 4, "drippy plum(?)" )
					.turnCost( 1 )
					.attachItem( ItemPool.DRIPPY_PLUM, 1, AUTO );
				new Option( 5, "drippy truncheon -> drippy stake" )
					.turnCost( 1 )
					.attachItem( ItemPool.DRIPPY_TRUNCHEON, new DisplayAll( "truncheon", NEED, EQUIPPED_AT_LEAST, 1 ) )
		...
		{
			final Pattern MUSHROOM_COSTUME_PATTERN = Pattern.compile( "<form.*?name=option value=(\\d).*?type=submit value=\"(.*?) Costume\".*?>(\\d+) coins<.*?</form>" );
			...
			{
				String costume;
				if ( decision == 1 && request.responseText.contains( "You slip into something a little more carpentable" ) )
				{
					costume = "muscle";
				}
				else if ( decision == 2 && request.responseText.contains( "You let down your guard and put on the gardener costume" ) )
				{
					costume = "mysticality";
				}
				else if ( decision == 3 && request.responseText.contains( "Todge holds out a tutu and you jump into it" ) )
				{
					costume = "moxie";
				}
				else
		...
		new ChoiceAdventure( 1411, "The Hall in the Hall", "The Dripping Hall" )
		{
			...
			void setup()
			{
				...
				new Option( 1, "drippy pool table", true )
					.turnCost( 1 )
					.attachItem( ItemPool.DRIPPY_STAFF, 1, AUTO, new DisplayAll( "don't have", WANT, EXACTLY, 0 ) )
					.attachItem( "drippy orb", 1, AUTO, new DisplayAll( "drippy orb" ) );
				new Option( 2 )
					.leadsTo( 1415, false, ( Option o ) -> true, " or " );
				new Option( 3, "drippy humanoid", true )
					.turnCost( 1 );
				...
				new Option( 5, "Get some Driplets", true )
					.turnCost( 1 )
					.attachItem( ItemPool.DRIPLET, 3, AUTO );
		...
		new ChoiceAdventure( 1412, "Guzzlr Client Selection", "Item-Driven" )
		{
			...
			final Pattern GUZZLR_TIER_PATTERN = Pattern.compile( "You have completed ([0-9,]+) (Bronze|Gold|Platinum) Tier deliveries" );
			final Pattern GUZZLR_QUEST_PATTERN = Pattern.compile( "<p>You are currently tasked with taking a (.*?) to (.*?) in (.*?)\\.<p>" );
			// You select a Gold Tier client, Norma "Smelly" Jackson, a hobo from The Oasis.
			final Pattern GUZZLR_LOCATION_PATTERN = Pattern.compile( "You select a (Bronze|Gold|Platinum) Tier client, (.*?), +an? (.*?) from (.*?)\\.<p>" );
			...
			void visitChoice( GenericRequest request )
			{
				...
				if ( request.responseText.contains( "You are currently tasked with" ) )
				{
					...
					if ( alreadyMatcher.find() )
					{
						...
						Preferences.setString( "guzzlrQuestBooze", booze.equals( "special personalized cocktail" ) ?
							"Guzzlr cocktail set" : booze );
					...
					Preferences.setBoolean( "_guzzlrQuestAbandoned", ChoiceManager.findChoiceDecisionIndex( "Abandon Client", request.responseText ) == "0" );
				...
				if ( unlockedGoldTier && ChoiceManager.findChoiceDecisionIndex( "Gold Tier", request.responseText ) == "0" )
				{
				...
				if ( unlockedPlatinumTier && ChoiceManager.findChoiceDecisionIndex( "Platinum Tier", request.responseText ) == "0" )
				{
		...
		new ChoiceAdventure( 1414, "Lock Picking", null )
		{
			...
			{
				new Option( 1 )
					.attachItem( ItemPool.BORIS_KEY, 1, AUTO );
				new Option( 2 )
					.attachItem( ItemPool.JARLSBERG_KEY, 1, AUTO );
				new Option( 3 )
					.attachItem( ItemPool.SNEAKY_PETE_KEY, 1, AUTO );
			}
		...
		new ChoiceAdventure( 1418, "So Cold", "Item-Driven" )
		{
			void setup()
			{
				new Option( 1, "10 adv of Entauntauned (+1 cold resist per Melodramedary weight), resets Melodramedary's xp to 0" )
		...
		new ChoiceAdventure( 1427, "The Hidden Junction", "Guano Junction" )
		{
			void setup()
			{
				...
				new Option( 2, "gain ~360 meat", true )
					.turnCost( 1 )
					.attachMeat( 300, AUTO );
			}
		...
		new ChoiceAdventure( 1428, "Your Neck of the Woods", "The Dark Neck of the Woods" )
		{
			void setup()
			{
				...
				new Option( 2, "advance quest 2 steps", true )
					.turnCost( 1 );
			}
		...
		new ChoiceAdventure( 1429, "No Nook Unknown", "The Defiled Nook" )
		{
			void setup()
			{
				...
				new Option( 1, "acquire 2 evil eyes (0)", true )
					.attachItem( ItemPool.EVIL_EYE, 2, AUTO );
		...
		new ChoiceAdventure( 1430, "Ghostly Memories", "A-Boo Peak" )
		{
			void setup()
			{
				...
				new Option( 1, "the Horror, spooky/cold res recommended", true )
					.leadsTo( 611 );
				new Option( 2, "fight oil baron", true );
				new Option( 3, "lost overlook lodge", true )
					.leadsTo( 606, true );
			}
		...
		new ChoiceAdventure( 1431, "Here There Be Giants", "The Castle in the Clouds in the Sky (Top Floor)" )
		{
			void setup()
			{
				this.isSuperlikely = true;
				...
				new Option( 1 )
					.leadsTo( 677, true, ( Option o ) -> true );
				new Option( 2 )
					.leadsTo( 675, true, ( Option o ) -> true );
				new Option( 3 )
					.leadsTo( 676, true, ( Option o ) -> true );
				new Option( 4 )
					.leadsTo( 678, true, ( Option o ) -> true );
		...
		new ChoiceAdventure( 1433, "Sneaky, Sneaky", "Wartime Hippy Camp (Frat Disguise)" )
		{
			void setup()
			{
				...
				new Option( 1 )
					.leadsTo( 139, false, ( Option o ) -> true );
				new Option( 2 )
					.leadsTo( 140, false, ( Option o ) -> true );
				new Option( 3 );
		...
		new ChoiceAdventure( 1434, "Sneaky, Sneaky", "Wartime Frat House (Hippy Disguise)" )
		{
			void setup()
			{
				...
				new Option( 1 )
					.leadsTo( 143, false, ( Option o ) -> true );
				new Option( 2 );
				new Option( 3 )
					.leadsTo( 144, false, ( Option o ) -> true );
		...
		{
			final Pattern MAPPED_MONSTER_PATTERN = Pattern.compile( "(<input type=\"hidden\" name=\"heyscriptswhatsupwinkwink\" value=\"(\\d+)\" />\\s+<input type=\"submit\" class=\"button\" value=\").*?(\" />\\s+</form>)" );
		...
		new ChoiceAdventure( 1436, "Billiards Room Options", "The Haunted Billiards Room" )
		{
			void setup()
			{
				...
				new Option( 1, "get the pool cue", true )
					.turnCost( 1 )
					.attachItem( ItemPool.POOL_CUE, 1, AUTO );
				new Option( 2, "go straight to the ghost", true )
					.leadsTo( 875 );
		...
		new ChoiceAdventure( 1437, "Setup Your knock-off retro superhero cape", "Item-Driven" ) // washing instructions
		{
			void setup()
			{
				this.canWalkFromChoice = true;
		...
		new ChoiceAdventure( 1438, "Setup Your knock-off retro superhero cape", "Item-Driven" ) // superhero
		{
			void setup()
			{
				this.canWalkFromChoice = true;
		...
		new ChoiceAdventure( 1442, "Shipping Government Food", "Item-Driven" )
		{
			void setup()
			{
				...
				new Option( 1 )
					.attachItem( ItemPool.GOVERNMENT_FOOD_SHIPMENT, -1, MANUAL, new NoDisplay() );
			}

			@Override
			void postChoice1( GenericRequest request, int decision )
			{
				if ( decision == 1 && !request.responseText.contains( "You fill out all the appropriate forms" ) )
				{
					this.choiceFailed();
				}
			}
		...
		new ChoiceAdventure( 1443, "Shipping Government Booze", "Item-Driven" )
		{
			void setup()
			{
				...
				new Option( 1 )
					.attachItem( ItemPool.GOVERNMENT_BOOZE_SHIPMENT, -1, MANUAL, new NoDisplay() );
			}
		...
		new ChoiceAdventure( 1444, "Shipping Government Candy", "Item-Driven" )
		{
			void setup()
			{
				...
				new Option( 1 )
					.attachItem( ItemPool.GOVERNMENT_CANDY_SHIPMENT, -1, MANUAL, new NoDisplay() );
			}
		...
		{
			//<button type="submit" name="pp" value="6" (...)> <img (...) src="https://s3.amazonaws.com/images.kingdomofloathing.com/otherimages/powerplant/2.png" alt="battery (AA)" title="battery (AA)"> </button>
			final Pattern STALK_PATTERN = Pattern.compile( "<button.*?name=\"pp\" value=\"(\\d+)\".*?>\\s+<img.*?src=\".*?/otherimages/powerplant/(\\d+)\\.png\"" );
		...
		new ChoiceAdventure( 1449, "Set Backup Camera Mode", "Item-Driven" )
		{
			...
			void visitChoice( GenericRequest request )
			{
				String setting = !request.responseText.contains( "Warning Beep" ) ? "ml" :
				                 !request.responseText.contains( "Infrared Spectrum" ) ? "meat" :
				                 !request.responseText.contains( "Maximum Framerate" ) ? "init" : "";

				Preferences.setString( "backupCameraMode", setting );
				Preferences.setBoolean( "backupCameraReverserEnabled", request.responseText.contains( "Disable Reverser" ) );
			}
