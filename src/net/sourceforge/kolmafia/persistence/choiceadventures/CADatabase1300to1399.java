	...
	final void add1300to1399()
	{
		...
		new ChoiceAdventure( 1303, "Ley Lady Ley", "The Ley Nexus" )
		{
			void setup()
			{
				...
				new CustomOption( 1, "fight Ley Incursion (with 500+ mys and Cheswick Copperbottom's compass)" );
			}
		...
		new ChoiceAdventure( 1304, "He Is the Ghoul King, He Can Do Anything", "The Ghoul King's Catacomb" )
		{
			void setup()
			{
		...
		new ChoiceAdventure( 1307, "It Takes a Thief", "The Master Thief's Chalet" )
		{
			void setup()
			{
				...
				new CustomOption( 1, "fight Ted Schwartz, Master Thief (with 5+ sleaze res and notarized arrest warrant)" );
			}
		...
		{
			final Pattern MUFFIN_TYPE_PATTERN = Pattern.compile( "Looks like your order for a (.*? muffin) is not yet ready" );
			final Pattern CHOICEFORM_PATTERN = Pattern.compile( "name=choiceform\\d+(.*?)</form>", Pattern.DOTALL );
			...
			void visitChoice( GenericRequest request )
			{
				...
				while ( matcher.find() )
				{
					...
					case "Exchange 10 shovelfuls of dirt and 10 hunks of granite for an earthenware muffin tin!":
						option.attachItem( ItemPool.SHOVELFUL_OF_DIRT, -10, MANUAL )
					...
					case "Order a blueberry muffin":
						option.attachItem( ItemPool.BLUEBERRY_MUFFIN, 1, AUTO )
							.attachItem( ItemPool.EARTHENWARE_MUFFIN_TIN, -1, MANUAL, new NoDisplay() );
						break;
					case "Order a bran muffin":
						option.attachItem( ItemPool.BRAN_MUFFIN, 1, AUTO )
							.attachItem( ItemPool.EARTHENWARE_MUFFIN_TIN, -1, MANUAL, new NoDisplay() );
						break;
					case "Order a chocolate chip muffin":
						option.attachItem( ItemPool.CHOCOLATE_CHIP_MUFFIN, 1, AUTO )
							.attachItem( ItemPool.EARTHENWARE_MUFFIN_TIN, -1, MANUAL, new NoDisplay() );
						break;
		...
		new ChoiceAdventure( 1312, "Choose a Soundtrack", "Item-Driven" )
		{
			...
			final Pattern BOOMBOX_SONG_PATTERN = Pattern.compile( "&quot;(.*?)&quot;( \\(Keep playing\\)|)" );
		...
		{
			final Pattern HEIST_PATTERN = Pattern.compile( "He shows you a list of potential targets:<p><i>\\((\\d+) more" );
			//<p>From a basaltamander:<br><input type="submit" name="st:1799:8465" value="basaltamander tongue" class="button"></p>
		...
		new ChoiceAdventure( 1322, "The Beginning of the Neverend", "The Neverending Party" )
		{
			...
			void visitChoice( GenericRequest request )
			{
				...
				else if ( request.responseText.contains( "helping her with whatever problem she's having with the snacks" ) )
				{
		...
		new ChoiceAdventure( 1324, "It Hasn't Ended, It's Just Paused", "The Neverending Party" )
		{
			void setup()
			{
				...
				new Option( 1 )
					.leadsTo( 1325, true, ( Option o ) -> true );
				new Option( 2 )
					.leadsTo( 1326, true, ( Option o ) -> true );
				new Option( 3 )
					.leadsTo( 1327, true, ( Option o ) -> true );
				new Option( 4 )
					.leadsTo( 1328, true, ( Option o ) -> true );
		...
		new ChoiceAdventure( 1325, "A Room With a View...  Of a Bed", "The Neverending Party" )
		{
			...
			void setup()
			{
				...
				new Option( 1 );
				new Option( 2, "20 adv of Tomes of Opportunity" )
					.attachEffect( "Tomes of Opportunity" );
				...
				new Option( 5, "+20 megawoots" )
					.attachItem( ItemPool.VERY_SMALL_RED_DRESS, -1, MANUAL, new DisplayAll( NEED, AT_LEAST, 1 ) );
			...
			void visitChoice( GenericRequest request )
			{
				...
				{
					getOption( 1 )
						.text( this.customOptions.get( 1 ).displayText );
		...
		{
			final Pattern GERALDINE_PATTERN = Pattern.compile( "Geraldine wants (\\d+)<table>.*?descitem\\((\\d+)\\)" );
			...
			void setup()
			{
				...
				new Option( 2, "20 adv of Spiced Up" )
					.attachEffect( "Spiced Up" );
			...
			void visitChoice( GenericRequest request )
			{
				...
				if ( quest.equals( "food" ) )
				{
					...
					if ( position > 0 )
					{
						...
						{
							getOption( 4 ).attachItem( StringUtilities.parseInt( itemIdString ), -10, MANUAL, new DisplayAll( NEED, AT_LEAST, 10 ) );
						}
					}
				}
		...
		{
			final Pattern GERALD_PATTERN = Pattern.compile( "Gerald wants (\\d+)<table>.*?descitem\\((\\d+)\\)" );
			...
			void setup()
			{
				...
				new Option( 2, "50 adv of Citronella Armpits" )
					.attachEffect( "Citronella Armpits" );
				new Option( 3, "find out booze to collect", true );
				new Option( 4, "give collected booze", true );
				new Option( 5, "remove 20% of partiers" )
					.attachItem( ItemPool.PURPLE_BEAST_ENERGY_DRINK, -1, MANUAL, new DisplayAll( NEED, AT_LEAST, 1 ) );
				...
				if ( quest.equals( "booze" ) )
				{
					...
					if ( position > 0 )
					{
						...
						if ( itemIdString != null )
						{
							...
							getOption( 4 ).attachItem( StringUtilities.parseInt( itemIdString ), -needed, MANUAL, new DisplayAll( NEED, AT_LEAST, needed ) );
						}
					}
				}
		...
		{
			final Pattern VOTE_PATTERN = Pattern.compile( "<label><input .*? value=\\\"(\\d)\\\" class=\\\"locals\\\" /> (.*?)<br /><span .*? color: blue\\\">(.*?)</span><br /></label>" );
			final Pattern VOTE_SPEECH_PATTERN = Pattern.compile( "<p><input type='radio' name='g' value='(\\d+)' /> <b>(.*?)</b>(.*?)<br><blockquote>(.*?)</blockquote>" );
		...
		new ChoiceAdventure( 1332, "government requisition form", "Item-Driven" )
		{
			void setup()
			{
				...
				new Option( 1 )
					.attachItem( ItemPool.GOVERNMENT_REQUISITION_FORM, -1, MANUAL, new NoDisplay() );
			}
		};
		...
		new RetiredChoiceAdventure( 1333, "Canadian cabin", "The Canadian Wildlife Preserve" )
		{
			void setup()
			{
				...
				new Option( 10, "skip adventure", true )
					.entersQueue( false );
		...
		new ChoiceAdventure( 1334, "Boxing Daycare (Lobby)", null )
		{
			void setup()
			{
				...
				new Option( 2, "get a daily 100 turns buff" )
					.leadsTo( 1335 );
				new Option( 3 )
					.leadsTo( 1336 );
				new Option( 4 );
			...
			void postChoice2( GenericRequest request, int decision )
			{
				...
				else if ( decision == 2 && request.responseText.contains( "only allowed one spa treatment" ) )
				{
		...
		new ChoiceAdventure( 1335, "Boxing Day Spa", null )
		{
			void setup()
			{
				...
				new CustomOption( 4, "100 adv of +100 max hp, +50 max mp, +25 dr, 5-10 mp regen, 10-20 hp regen" );
			}
		...
		{
			final Pattern EARLY_DAYCARE_PATTERN = Pattern.compile( "mostly empty. (.*?) toddlers are training with (.*?) instructor" );
			final Pattern DAYCARE_PATTERN = Pattern.compile( "(?:Looks like|Probably around) (.*?) pieces in all. (.*?) toddlers are training with (.*?) instructor" );
			final Pattern DAYCARE_RECRUITS_PATTERN = Pattern.compile( "<font color=blue><b>\\[(.*?) Meat\\]</b></font>" );
			final Pattern DAYCARE_RECRUIT_PATTERN = Pattern.compile( "attract (.*?) new children" );
			final Pattern DAYCARE_EQUIPMENT_COST_PATTERN = Pattern.compile( "<font color=blue><b>\\[(free|\\d adventures?)\\]</b></font>" );
			final Pattern DAYCARE_EQUIPMENT_PATTERN = Pattern.compile( "manage to find (.*?) used" );
			final Pattern DAYCARE_INSTRUCTOR_COST_PATTERN = Pattern.compile( "Hire an instructor.*?<font color=blue><b>\\[(\\d+) (.*?)\\]</b></font>" );
			final Pattern DAYCARE_ITEM_PATTERN = Pattern.compile( "<td valign=center>You lose an item: </td>.*?<b>(.*?)</b> \\((.*?)\\)</td>" );
			...
			void visitChoice( GenericRequest request )
			{
				...
				if ( scavengeCostMatcher.find() )
				{
					...
					int cost = costString.equals( "free" ) ? 0 :
					...
					if ( savedScavenges != cost &&
					     !( cost == 3 && savedScavenges > cost ) )
					{
						Preferences.setInteger( "_daycareGymScavenges", cost );
					}
				}

				Matcher instructorCostMatcher = DAYCARE_INSTRUCTOR_COST_PATTERN.matcher( request.responseText );
		...
		{
			final Pattern SAUSAGE_PATTERN = Pattern.compile( "grinder needs (.*?) of the (.*?) required units of filling to make a sausage.  Your grinder reads \\\"(\\d+)\\\" units." );
		...
		{
			final Pattern DOCTOR_BAG_PATTERN = Pattern.compile( "We've received a report of a patient (.*?), in (.*?)\\." );
			...
			void postChoice1( GenericRequest request, int decision )
			{
				if ( decision == 2 )
				{
					...
					for ( int i = 10; i < 39; i++ )
					{
						...
						else if ( !request.getURLString().contains( "sk[]=" + i ) && KoLCharacter.hasSkill( 24000 + i ) )
						{
		...
		new ChoiceAdventure( 1345, "Blech House", "The Smut Orc Logging Camp" )
		{
			void setup()
			{
				new Option( 1, "use muscle/weapon damage", true )
					.turnCost( 1 );
				new Option( 2, "use myst/spell damage", true )
					.turnCost( 1 );
				new Option( 3, "use mox/sleaze res", true )
					.turnCost( 1 );
			}
		...
		new ChoiceAdventure( 1347, "Groggy's Tavern", null )
		{
			...
			// Coxswain
			//"></form>
			final Pattern CREW_PATTERN = Pattern.compile( "<form.*?action=choice\\.php.*?name=option value=(\\d+).*?type=submit.*?value=\"the ([\\w\\-]+).+?(\\w+).\".*?</form>", Pattern.DOTALL );
			...
			void visitChoice( GenericRequest request )
			{
				...
				while ( crewMatcher.find() )
				{
					...
					{
						spoilerText.append( "+0-1 grub per fight, Avast, a Mast! gives +1 grub, helps when grub runs out" );
					}
		...
		new ChoiceAdventure( 1348, "Seaside Curios", null )
		{
			void setup()
			{
				new Option( 1, "(2-handed) fighting in A Sea Monster! always gives 10 fun and +300-400 gold (if worn)" )
					.leadsTo( 1349 )
					.attachItem( "bloody harpoon", 1, AUTO, new ImageOnly() );
				new Option( 2, "can be sold for 500 gold. If sold, Avast, a Mast! \"gives it back\", unlocking Glass Jack's Hideout" )
					.leadsTo( 1349 )
		...
		new ChoiceAdventure( 1349, "Dishonest Ed's Ships", null )
		{
			void setup()
			{
				new Option( 1, "7 turns between islands, 3 guns, 10 grub, 10 grog, 1 glue" )
					.leadsTo( 1350 );
				new Option( 2, "7 turns between islands, 4 guns, 8 grub, 8 grog, 0 glue" )
					.leadsTo( 1350 );
				new Option( 3, "6 turns between islands, 1 gun, 5 grub, 5 grog, 1 glue" )
					.leadsTo( 1350 );
				new Option( 4, "4 turns between islands, 0 gun, 5 grub, 5 grog, 1 glue" )
					.leadsTo( 1350 );
				new Option( 5, "9 turns between islands, 8 guns, 5 grub, 30 grog, 0 glue" )
					.leadsTo( 1350 );
			}
		...
		new ChoiceAdventure( 1352, "Island #1, Who Are You?", "Sailing the PirateRealm Seas" )
		{
			void setup()
			{
				...
				new Option( 1, "+3 fun & +1 grub per fight, foes drop 120-180 meat, boss drops 1600-2400 meat & gives 17 fun" );
		...
		new ChoiceAdventure( 1353, "What's Behind Island #2?", "Sailing the PirateRealm Seas" )
		{
			void setup()
			{
				...
				new Option( 2, "+4 fun & +6-11 gold per fight. With tomb opener, gives 20 fun & unlocks Red Roger's Fortress, else gives 10 fun" )
				...
				new Option( 5, "+3 fun & +1 grub & +1 grog per fight, foes drop 400-600 meat, gives 4 random tradeable items (2-3 of each)" );
			}
		...
		{
			final AdventureResult ROGER_LEFT_FOOT = new AdventureResult( "Red Roger's red left foot", 0, false );
			final AdventureResult ROGER_RIGHT_FOOT = new AdventureResult( "Red Roger's red right foot", 0, false );
			final AdventureResult ROGER_LEFT_HAND = new AdventureResult( "Red Roger's red left hand", 0, false );
			final AdventureResult ROGER_RIGHT_HAND = new AdventureResult( "Red Roger's red right hand", 0, false );
			...
			void visitChoice( GenericRequest request )
			{
				...
				for ( AdventureResult piece : new AdventureResult[] { ROGER_LEFT_FOOT, ROGER_RIGHT_FOOT, ROGER_LEFT_HAND, ROGER_RIGHT_HAND } )
				{
				...
				{
					getOption( 7 ).text( "exit and return with all 4 of Red Roger's parts to unlock Temple Island<br><br><br><br>" )
						.attachItem( ROGER_RIGHT_HAND, new DisplayAll( "(?:<br>){1}", WANT, EQUIPPED_AT_LEAST, 1 ) )
						.attachItem( ROGER_LEFT_HAND, new DisplayAll( "(?:<br>){2}", WANT, EQUIPPED_AT_LEAST, 1 ) )
						.attachItem( ROGER_RIGHT_FOOT, new DisplayAll( "(?:<br>){3}", WANT, EQUIPPED_AT_LEAST, 1 ) )
						.attachItem( ROGER_LEFT_FOOT, new DisplayAll( "(?:<br>){4}", WANT, EQUIPPED_AT_LEAST, 1 ) );
				}
			}
		...
		new ChoiceAdventure( 1356, "Smooth Sailing", "Sailing the PirateRealm Seas" )
		{
			void setup()
			{
				new Option( 1, "-1 grub, +5 fun", true )
					.turnCost( 1 );
				new Option( 2, "-1 grog, +5 fun", true )
					.turnCost( 1 );
				new Option( 3, "+3 fun", true )
					.turnCost( 1 );
			}
		...
		new ChoiceAdventure( 1357, "High Tide, Low Morale", "Sailing the PirateRealm Seas" )
		{
			void setup()
			{
				new Option( 1, "-3 grub, +5 fun", true )
					.turnCost( 1 );
				new Option( 2, "-3 grog, +5 fun", true )
					.turnCost( 1 );
				new Option( 3, "-30 gold, +5 fun", true )
					.turnCost( 1 );
				new Option( 4, "+1 fun, +1 fun (yes, twice)", true )
					.turnCost( 1 );
			}
		...
		new ChoiceAdventure( 1358, "The Starboard is Bare", "Sailing the PirateRealm Seas" )
		{
			void setup()
			{
				new Option( 1, "+5 grub, +1 fun", true )
					.turnCost( 1 );
				new Option( 2, "+1-8 grub, +3 fun", true )
					.turnCost( 1 );
			}
		...
		new ChoiceAdventure( 1359, "Grog for the Grogless", "Sailing the PirateRealm Seas" )
		{
			void setup()
			{
				new Option( 1, "+5 grog, +1 fun", true )
					.turnCost( 1 );
				new Option( 2, "+1-8 grog, +3 fun", true )
					.turnCost( 1 );
			}
		...
		new ChoiceAdventure( 1360, "Like Shops in the Night", "Sailing the PirateRealm Seas" )
		{
			...
			{
				if ( decision == 5 &&
				     request.responseText.contains( "You gain 500 gold" ) )
				{
		...
		{
			final AdventureResult CURIOUS_ANEMOMETER = new AdventureResult( "curious anemometer", 0, false );

			void setup()
			{
				new Option( 1, "+5 fun", true )
					.turnCost( 1 );
				new Option( 2, "+8 fun. Either skip a sea non-combat, or ship broken", true )
					.turnCost( 1 );
			}
			...
			void visitChoice( GenericRequest request )
			{
				...
				getOption( 2 ).text( "+8 fun. Either skip a sea non-combat, or ship broken. Chance of skipping: " + likelihood );
			}
		...
		new ChoiceAdventure( 1363, "Who Pirates the Pirates?", "Sailing the PirateRealm Seas" )
		{
			void setup()
			{
				new Option( 1, "+8 fun and +50-60 gold and may gain 1 gun, or +7 fun and (-3 grub or -3 grog or -25-30 gold or ship broken)", true )
					.turnCost( 1 );
				new Option( 2, "+5 fun, may lose 1 grub or grog", true )
					.turnCost( 1 );
			}
			...
			void visitChoice( GenericRequest request )
			{
				...
				{
					getOption( 1 ).text( "+8 fun and +80-100 gold and may gain 1 gun, or +7 fun and (-3 grub or -3 grog or -25-30 gold or ship broken)" );
				}
			}
		...
		new ChoiceAdventure( 1364, "An Opportunity for Dastardly Do", "Sailing the PirateRealm Seas" )
		{
			void setup()
			{
				new Option( 1, "+6 fun and (+1 gun or +3-10 grub or +3-10 grog or +50-60 gold), or +4 fun and ship broken", true )
					.turnCost( 1 );
				new Option( 2, "+3 fun", true )
					.turnCost( 1 );
			}
		...
		new ChoiceAdventure( 1367, "The Ship is Wrecked", "Sailing the PirateRealm Seas" )
		{
			void setup()
			{
				new Option( 1, "+5 fun, -1 glue", true )
					.turnCost( 1 );
				new Option( 2, "+1 fun", true )
					.turnCost( 1 );
			}
		...
		new ChoiceAdventure( 1391, "Rationing out Destruction", "The Exploaded Battlefield" )
		{
			...
			final Pattern FRAT_RATIONING_PATTERN = Pattern.compile( "You toss the (.*?) off the edge of the floating battlefield and (\\d+) frat boys? jumps? off after it." );
			// You toss the mana curds into the crowd.  10 hippies dive onto it, greedily consume it, and pass out.
			final Pattern HIPPY_RATIONING_PATTERN = Pattern.compile( "You toss the (.*?) into the crowd.  (\\d+) hippies dive onto it, greedily consume it, and pass out." );
		...
		new ChoiceAdventure( 1394, "Send up a Smoke Signal", "Item-Driven" )
		{
			...
			{
				new Option( 1 )
					.attachItem( ItemPool.CAMPFIRE_SMOKE, -1, MANUAL, new NoDisplay() );
			}
		...
		{
			final Pattern RED_SNAPPER_PATTERN = Pattern.compile( "guiding you towards: <b>(.*?)</b>.  You've found <b>(\\d+)</b> of them" );
		...
		new RetiredChoiceAdventure( 1397, "Kringle workshop", "The Wreck of the H. M. S. Kringle" )
		{
			void setup()
			{
				...
				new Option( 1, "craft stuff", true )
					.turnCost( 1 );
				new Option( 2, "get waterlogged items", true )
					.turnCost( 1 );
				new Option( 3, "fail at life", true )
					.turnCost( 1 );
			}
		};
